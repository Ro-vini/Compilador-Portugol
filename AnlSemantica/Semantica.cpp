...
int main() {
    ...
    int nivel_se = 0;
    int nivel_para = 0;
	bool comando_encontrado_no_para = false;
    ...
    while (getline(entrada, linha)) {
        ...
        // ------------------------------
        if (esta_na_linha(tokens, "TIPO") || esta_na_linha(tokens, "DEC")) {
            if (tokens.size() == 3 &&
                tokens[0].first == "TIPO" &&
                tokens[1].first == "DEC" &&
                tokens[2].first == "ID") {
        ...
        // ------------------------------
            ...
            if (tokens[1].first == "ID" || tokens[1].first == "NUMINT") {
                ...
                log_erro(log, "Erro: Variavel ou valor esperado após a palavra reservada 'se'.");
                erro_encontrado = true;
			}
            ...
            if (tokens[3].first == "ID" || tokens[3].first == "NUMINT") {
                ...
                log_erro(log, "Erro: Variavel ou valor esperado após o operador de comparação.");
                erro_encontrado = true;
            }

            if (tokens[4].first != "ENTAO") {
                log_erro(log, "Erro: 'entao' esperado após condição em 'se'.");
                erro_encontrado = true;
            }
        }
            ...
            if (primeiro_token != "FIMPARA" || tokens.size() != 1) {
                log_erro(log, "Erro: 'fim_para' deve ser sozinho na linha.");
                erro_encontrado = true;
            }
